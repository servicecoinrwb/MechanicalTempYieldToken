/**
 *Submitted for verification at Arbiscan.io on 2026-04-04
*/
...
// File: @openzeppelin/contracts/utils/Memory.sol
...
// File: MtAutoSafeVault.sol


pragma solidity ^0.8.25;

// ✅ Fixed: Removed @5.0.2 version pins — Remix resolves these via npm automatically




/**
 * @title MtAutoSafeVault
 * @notice A "Safe" ETH accumulator for mtyld holders on Arbitrum.
 */
contract MtAutoSafeVault is ERC4626, Ownable, ReentrancyGuard {
    ...
    uint256 public constant WITHDRAW_LOCK_PERIOD = 24 hours;
    ...
    event StrategyApproved(address indexed target, bool status);
    event StrategyExecuted(address indexed target, uint256 totalAssetsAfter);

    constructor(IERC20 _mtyld) 
        ERC4626(IERC20(WETH)) 
        ERC20("mtAUTO Safe ETH", "mtETH-S") 
        Ownable(msg.sender) 
    {
        mtyld = _mtyld;
        aiAgent = msg.sender; 
    ...
    function approveStrategy(address _target, bool _status) external onlyOwner {
        isApprovedStrategy[_target] = _status;
        emit StrategyApproved(_target, _status);
    }

    // --- CORE LOGIC WITH TOKEN GATE ---

    function deposit(uint256 assets, address receiver) public override nonReentrant returns (uint256) {
        require(mtyld.balanceOf(msg.sender) > 0, "RWA Token Required");
    ...
    function withdraw(uint256 assets, address receiver, address owner) public override returns (uint256) {
        require(block.timestamp >= lastDepositTime[owner] + WITHDRAW_LOCK_PERIOD, "Withdrawal locked");
        return super.withdraw(assets, receiver, owner);
    }

    // --- THE "CONSERVATOR" AI STRATEGY ---

    function executeStrategy(address target, bytes calldata data) external onlyAgent nonReentrant {
        require(isApprovedStrategy[target], "Target not on allowlist");
        
        (bool success, ) = target.call(data);
        require(success, "Strategy execution failed");
        
        emit StrategyExecuted(target, totalAssets());
    }
